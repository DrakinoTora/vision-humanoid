...
def detect(mask_ball, mask_field, frame):
    ...
    if circles is not None:
        ...
        for i in circles[0, :]:
            ...
            for idx in range(orange_hline.shape[0]): 
                if orange_hline[idx] == 255:
                    first_orange = line_x_start + idx 
                    break

            for idx in range(orange_hline.shape[0] - 1, -1, -1):  
                if orange_hline[idx] == 255:
                    last_orange = line_x_start + idx 
                    break
            ...
            #cv2.line(frame, (x_new, line_y_start), (x_new, line_y_end), (0, 255, 0), 2)

            top_orange = None
            bot_orange = None

            for idy in range(orange_vline.shape[0]): 
                if orange_vline[idy] == 255:
                    top_orange = line_y_start + idy 
                    break

            for idy in range(orange_vline.shape[0] - 1, -1, -1):  
                if orange_vline[idy] == 255:
                    bot_orange = line_y_start + idy 
                    break
            ...
            #text3 = f"ball: {ball_ratio:.2f}"
            #text4 = f"field: {field_ratio:.2f}"
            cv2.putText(frame, text2, (x_new - r_new, y_new + r_new + 40), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
            #cv2.putText(frame, text3, (x, y), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
            #cv2.putText(frame, text4, (x, y + 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
...
img = cv2.imread(r"D:\GMRT\Altair\vision\Program deteksi bola\sample3.jpg")
